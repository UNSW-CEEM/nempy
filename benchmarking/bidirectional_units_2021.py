...
def get_test_intervals(number=100):
    ...
    end_time = datetime(year=2021, month=1, day=31, hour=0, minute=0)
...
for interval in get_test_intervals(number=100):
    ...
    market.set_unit_ramp_rate_constraints(ramp_rates, fast_start_profiles,
    ...
    if 'bidirectional_ramp_up' in market.get_constraint_set_names():
        market.make_constraints_elastic('bidirectional_ramp_up',
                                        violation_cost=cost)
        market.make_constraints_elastic('bidirectional_ramp_down',
                                        violation_cost=cost)
    ...
    market.set_unit_ramp_rate_constraints(
        ramp_rates, fast_start_profiles, run_type="fast_start_second_run")
    cost = constraint_inputs.get_constraint_violation_prices()['ramp_rate']
    market.make_constraints_elastic('ramp_up', violation_cost=cost)
    market.make_constraints_elastic('ramp_down', violation_cost=cost)
    if 'bidirectional_ramp_up' in market.get_constraint_set_names():
