...
def test_fcas_trapezium_scaled_availability():
    ...
    for interval in get_test_intervals():
        ...
        for unit in list(DISPATCHLOAD['DUID']):
            ...
            if 'raise_reg' not in list(fcas_trapeziums['service']):
                continue
            print(unit)
            ...
            if not unit_limits_one.empty:
                ...
                market.set_unit_ramp_up_constraints(unit_limits_one.loc[:, ['unit', 'initial_output', 'ramp_up_rate']])
                market.set_unit_ramp_down_constraints(unit_limits_one.loc[:, ['unit', 'initial_output', 'ramp_down_rate']])
            ...
            market.objective_function_components['bids']['cost'] = np.where(
                market.objective_function_components['bids']['service'] == 'raise_reg', -1.0, 0.0)
            ...
            decision_variables_test_service = decision_variables[decision_variables['service'] == 'raise_reg']
            decision_variables_not_test_service = decision_variables[~(decision_variables['service'] == 'raise_reg')]
            ...
            availabilities = availabilities[availabilities['service'] == 'raise_reg']
            ...
            availabilities['match'] = availabilities['error'] < 0.1
