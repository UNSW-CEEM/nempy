...
def add_losses(break_points, inter_variables, loss_functions, next_variable_id, next_constraint_id):
    ...
    next_variable_id = loss_variables['variable_id'].max()
    ...
    decision_variables = pd.concat([loss_variables, weight_variables])
    return decision_variables, lhs, weights_sum_rhs, dynamic_rhs
...
def link_weights_to_inter_loss(weight_variables, loss_variables, loss_functions, next_constraint_id):
    ...
    lhs['coefficient'] = lhs['loss_function'](lhs['break_point'])
...
def create_loss_variables(inter_variables, loss_function, next_variable_id):
    ...
    columns_for_loss_variables['lower_bound'] = 1
    loss_variables = pd.merge(loss_variables, inter_variables.loc[:, ['interconnector', 'coefficient']])
    loss_variables['coefficient'] = np.where(loss_variables['coefficient'] < 0.0,
                                             loss_variables['from_region_loss_share'],
