...
from nempy.historical_inputs import loaders, mms_db, \
    units, demand, interconnectors, constraints
...
raw_inputs_loader = loaders.RawInputsLoaderMMSOnly(
...
for interval in dispatch_intervals:
    ...
    unit_inputs = units.UnitDataMMSOnly(raw_inputs_loader)
    ...
    unit_availability = unit_inputs.get_unit_reported_availability()
    market.set_unit_bid_capacity_constraints(unit_availability)
    market.make_constraints_elastic('unit_bid_capacity', 20000.0)
    ...
    if unit_dispatch is None:
        ...
        ramp_rates = time_sequential.create_seed_ramp_rate_parameters(
            historical_dispatch, ramp_rates.loc[:, ['unit', 'ramp_up_rate', 'ramp_down_rate']])
    else:
...
print("Run time per dispatch {}".format(t_dispatch/len(dispatch_intervals)))
con.close()
