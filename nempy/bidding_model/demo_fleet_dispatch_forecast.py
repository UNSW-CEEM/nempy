...
axs.scatter(data['interval'], data['fleet_dispatch'], color='red', label='actual')
axs.legend()
fig.tight_layout()
...
data['error'] = (data['Y'] - data['fleet_dispatch'])
print(data['error'].abs().mean())

forcast = forcast[(-500.0 < forcast['Y']) & (forcast['Y'] < 500.0)]
data = data[(-500.0 < data['fleet_dispatch']) & (data['fleet_dispatch'] < 500.0)]

axs.hist(data['fleet_dispatch'], bins=50, color='red')
